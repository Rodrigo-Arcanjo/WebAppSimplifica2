...
namespace WebAppSimplifica2.Controllers
{
    ...
    [ApiController]
    public class RelatorioAnexo : ControllerBase
    {

        /// <summary>
        /// Obter as Coordenadas do Polígono
        /// </summary>
        /// <param name="processoNumero">Nº do Processo</param>
        ...
        [Authorize]
        [Route("IsProdTvl/{processoNumero}")]
        public IActionResult IsProdTvl(string processoNumero)
        {

            var processo = processoNumero.Replace("()_", "/");

            Status status = null;
            List<SqlParameter> lsql = new List<SqlParameter>
            {
                new SqlParameter("@PRO_NUMERO", processo)
            };
            ...
            string json = ConnectionHelper.ExecProcToJson($"SM_ISTVL", lsql);

            if (!string.IsNullOrWhiteSpace(json) && !json.Equals("[]"))
            {
                try
                {
                    status = new Status("OK", "", JsonConvert.DeserializeObject<List<RelatorioIsProdTvlDAO>>(json));
                }
                catch (Exception ex)
                {
